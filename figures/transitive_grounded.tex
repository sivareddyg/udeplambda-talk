...
thick,dashed,sloped,above,font=\sffamily\scshape,bend right=20},
  ...
  \matrix{
    \node[entityType] (tCompany) {organization.\\organization}; \& \node[mathType] (mCompany) {\textsc{target}}; \\
    
    \node[entity] (eCompany) {$x$}; \& \node[mediator] (e1) {$m$}; \& \node[entity] (ePixar)[font=\sc] {\scriptsize Pixar}; \\
  };
 
  % mediator to entities
  \draw [mediatorToEntity] (e1) edge node[below] {business.\\acquisition.\\acquiring\_company}  (eCompany);
  \draw [mediatorToEntity] (e1) edge node[below] {business.\\acquisition.\\company\_acquired} (ePixar);
