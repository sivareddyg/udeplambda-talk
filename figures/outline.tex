...
thick,dashed,violet,sloped,above,font=\sffamily\scriptsize},
  ...
  every node/.style={align=center,distance=2cm}]
   ...
   \node (sentence) [sent,right of=webcorpus, xshift=2cm,fill=white] {\includegraphics[trim=3.8em 0em 5em 6em,clip=true,scale=0.45]{corenlp}
   };
   \node (lf) [lf,right of=sentence, xshift=4cm,font=\tiny] {$\lambda x. \exists yz. \mathrm{name}(x_a) \wedge \mathrm{name(x_e)}$ \\$\mathrm{arg_0}(x_e, x_a) \mathrm{of}(x_e, y_a)$ \\ $\wedge \mathrm{company}(y_a) \wedge \mathrm{acquired}(z_e)$ \\ $ \wedge \mathrm{arg_1}(z_e, y_a) \wedge \mathrm{arg_2}(z_e, Pixar)$};
   \node (ug) [figg,below of=lf, yshift=-1cm] {\includegraphics[scale=0.5]{transitive_ungrounded_mismatch}};
   \node (db) [figg,left of=ug,xshift=-2cm,yshift=-1cm] {\includegraphics[scale=0.3]{db}};
   \node (gg) [figg,left of=ug,xshift=-7cm] {\includegraphics[scale=0.5]{transitive_grounded}};
   ...
   \draw [mediatorToEntity] (ug) -- node[red] {\Large Graph Transduction}  (gg);
