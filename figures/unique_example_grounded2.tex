...
\usepackage[papersize={8cm, 4cm}, text={8cm, 4cm}]{geometry}
...
(eTexas) {$\textsc{Texas}$}; \\ 
  ...
  \draw [mediatorToEntity][below] (mCapital) edge node {location.\\containedby.arg1}  (eAustin);
  \draw [mediatorToEntity][below] (mCapital) edge node {location.\\containedby.arg2}  (eTexas);
   
   \draw [entityToMath] (eAustin) edge node {}  (mTarget);
  
\end{tikzpicture} 
\scriptsize $\textsc{target}(x) \wedge \mbox{location.city}(x) \wedge 
\mbox{location.containedby.arg1}(n, x) \wedge
\mbox{location.containedby.arg2}(n, \textsc{Texas})$
\end{center}
