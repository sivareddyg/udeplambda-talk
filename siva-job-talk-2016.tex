...
\only<8>{\vspace{2cm}$\exists z. \mathrm{company(Pixar)} \wedge \mathrm{located}(z_e) \wedge \mathrm{arg_2}(z_e, \mathrm{Pixar}) \wedge \mathrm{arg_{in}}(z_e, \mathrm{CA})$\blfootnote{\scriptsize Reddy, T\"ackstr\"om, Kwiatkowski et al. (TACL 2016)}}
\end{center} 
\end{frame}
...
\vspace{-3.5cm} \\ \hspace{2cm} Latent
 };
\end{tikzpicture}}
\end{figure}
 \end{column}
...
\begin{frame}
  \frametitle{Semantic Parsing as Graph Matching\blfootnote{\scriptsize Reddy, Lapata, Steedman (TACL 2014)}}
...
\begin{frame}
  ...
  \begin{column}{0.42\textwidth}
    \centering
   \includegraphics[width=\textwidth]{figures/czech_question_grounded_graph} \\
   Freebase graph 
  \end{column}
 \end{columns}
...
\hlight{Structured Perceptron:} Ranks grounded and ungrounded graph pairs $$(\hat{g},\hat{u}) = \arg\max\limits_{g,u}
                        \Phi(g,u,q,\mathit{KB}) \cdot \theta$$

\pause
\hlight{Features $\Phi$:} Defined over question, grounded and
ungrounded graph
...
\pause
\hlight{Training:} Use gold graph to update weights 
$$\theta \leftarrow \theta +
...
\begin{itemize}
 \item But we \hlight{do not} have access to gold graphs
 \item Access only to the answers rather than the query
\end{itemize}
\end{frame}
